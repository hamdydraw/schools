		...
		@section('content')
			...
			<div id="page-wrapper">
					...
					<div class="row">
						<div class="col-lg-12">
							<ol class="breadcrumb">
								...
								<li>
									<a href="{{URL_MASTERSETTINGS_COURSE_SUBJECTS_ADD}}">{{ getPhrase('add_subject_to_course')}}</a>
								</li>
								<li>{{ $title }}</li>
							</ol>
						</div>
				...
				<!-- /.row -->
					...
					<input type="hidden" name="course_parent_id" value="{{$record->id}}">
					...
					<div class="panel panel-custom academia_visiblelist_fix" data-spy="affix" data-offset-top="0">
						...
						<div class="panel-body packages vertical-scroll" id="window_auto_height">
							...
							@for($yearno = 1; $yearno <= $record->course_dueration; $yearno++)
								...
								<div class="sem-parent-container  helper_step2">

									@if($record->is_having_semister)
                                        <?php
                                        $semisters = App\CourseSemister::getCourseYearSemisters($course_parent_id,
                                            $yearno);
                                        $total_data = [];
										...
										@for($semister=1; $semister <= $semisters->total_semisters; $semister++)
                                            ...
                                            <?php
                                            $subjects_list = App\CourseSubject::getCourseSavedSubjects($academic_id,
                                                $course_parent_id, $yearno, $semister);
										...
										@if($semisters->total_semisters == 0)
                                            <?php
                                            $subjects = App\CourseSubject::getCourseSavedSubjects($academic_id,
                                                $course_parent_id, $yearno, 0);

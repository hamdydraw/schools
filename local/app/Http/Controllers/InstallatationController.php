...
class InstallatationController extends Controller
{
   ...
   */
    ...
    function createTables(Request $request)
    {
        ...
        if($request->has('sample_data'))
        {
            // $query = file_get_contents(DOWNLOAD_EMPTY_DATA_DATABASE);
            $query = $this->get_content(DOWNLOAD_EMPTY_DATA_DATABASE);

            if($request->sample_data=='school')
                $query = $this->get_content(DOWNLOAD_SAMPLE_DATA_SCHOOL_DATABASE);
            elseif($request->sample_data=='college')
                $query = $this->get_content(DOWNLOAD_SAMPLE_DATA_COLLEGE_DATABASE);
            elseif($request->sample_data=='degree')
                $query = $this->get_content(DOWNLOAD_SAMPLE_DATA_DEGREE_DATABASE);
            elseif($request->sample_data=='pg')
                $query = $this->get_content(DOWNLOAD_SAMPLE_DATA_PG_DATABASE);

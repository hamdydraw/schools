...
class PaymentsController extends Controller
{
    ...
    public function __construct()
    {
        ...
        if (strpos(url()->current(), 'paynow') != false) {
            if (Input::all()['gateway'] == 'offline') {
                $this->middleware('stopOrOn:offline_payment');
            } else {
                $this->middleware('stopOrOn:' . Input::all()['gateway']);
            }
        }
    }
    ...
    public function approvePayment(Payment $payment_record, Request $request, $iscoupon_zero = 0)
    {
        ...
        if ($iscoupon_zero) {
            ...
            sendEmail($email_template, array(
                'username' => $user->name,
    ...
    public function approveOfflinePayment(Request $request)
    {
        ...
        try {
            if ($request->submit == 'approve') {
                ...
                $user = getUserRecord($payment_record->user_id);
                sendEmail('offline_subscription_failed', array(
                    'username' => $user->name,
                    'plan' => $payment_record->plan_type,
                    'to_email' => $user->email,
                    'admin_comment' => $request->admin_comment

...
class SessionGuard implements StatefulGuard, SupportsBasicAuth
{
    ...
    public function attempt(array $credentials = [], $remember = false, $login = true)
    {
        ...
        if ($this->hasValidCredentials($user, $credentials)) {
            if ($login) {
                //by mahmoud to check if detained user
                $lastStudentPromotion = StudentPromotion::where('user_id', $user->id)->orderBy('id',
                    'desc')->first(['type']);
                if ($lastStudentPromotion != null and $lastStudentPromotion->type === "detained") {
                    return 'detained';
                }
